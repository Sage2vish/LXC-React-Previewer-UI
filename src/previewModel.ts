import { buildModuleGraph, type Bundle } from './moduleBundler';

export type PreviewModel = {
  fileName: string;
  status: 'ready' | 'waiting';
  previewHtml: string;
  bundlePayloadJson: string;
...
export const IOS_VERSIONS: IosVersionProfile[] = [
...
export function buildPreviewModel(
  ...
  const status: PreviewModel['status'] = hasDefaultExport ? 'ready' : 'waiting';
  const platformOS: PreviewModel['platformOS'] =
    activeDevice.family === 'android' || activeDevice.family === 'android-tablet' ? 'android' : 'ios';

  let bundle: Bundle | undefined;
  if (status === 'ready') {
    bundle = buildModuleGraph(filePath, source);
  }
...
function renderChrome(device: DeviceProfile, iosVersionId: string | undefined, logoDataUri: string, status: PreviewModel['status']): string {
  const platformVersions = IOS_VERSIONS.filter((entry) => entry.family === device.family);
