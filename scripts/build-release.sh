...
if (command.title !== 'LXC - React Previewer') throw new Error(`Expected title LXC - React Previewer, got ${command.title}`);
if (command.shortTitle !== 'Preview') throw new Error(`Expected shortTitle Preview, got ${command.shortTitle}`);
if (!command.icon || command.icon.dark !== 'assets/icons-all-size/icon-16.png') throw new Error('Asset-backed icon not wired');
const refreshCommand = pkg.contributes?.commands?.find((entry) => entry.command === 'lxcReactPreviewer.refreshPreview');
if (!refreshCommand) throw new Error('refreshPreview command not found');
if (refreshCommand.title !== 'LXC - React Previewer : Refresh') throw new Error(`Expected refresh title LXC - React Previewer : Refresh, got ${refreshCommand.title}`);
