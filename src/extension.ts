...
import { buildPreviewModel, DEVICE_PROFILES, IOS_VERSIONS, type DeviceProfile, type PreviewModel } from './previewModel';
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const renderDocument = (document: vscode.TextDocument) => {
    ...
    state.panel.webview.html = renderHtml(
      buildPreviewModel(document.getText(), document.fileName, activeDevice, activeIosVersionId, BRAND_LOGO_URI),
  ...
  const handleWebviewMessage = (message: { type?: string; value?: string }) => {
    ...
    if (message.type === 'ios' && message.value) {
      const selected = IOS_VERSIONS.find((option) => option.id === message.value);
      if (!selected) {
        return;
      }
